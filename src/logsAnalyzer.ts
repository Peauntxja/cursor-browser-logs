 ...
 * 错误与文件的关联结果
 */
interface ErrorFileMapping {
    errorType: string;
    errorMessage: string;
    fileName: string;
    lineHint?: number;
    suggestion: string;
    confidence: number; // 0-1之间，表示信心程度
}
...
export class LogsAnalyzer {
     ...
     * 根据文件扩展名获取语言标识符
     */
    private getLanguageByExt(ext: string): string {
     ...
     * 分析错误类型
     */
    private analyzeErrorTypes(logs: any[]): { [key: string]: number } {
     ...
     * 生成建议
     */
    private generateSuggestions(logs: any[]): string[] {
        const suggestions: string[] = [];
        const errorLogs = logs.filter(log => log.level === 'error');
        ...
        if (errorLogs.length > 0) {
            ...
            for (const type in errorTypes) {
                if (errorTypes[type] > 0) {
                    const suggestion = this.getSuggestionForErrorType(String(type));
     ...
     * 识别关键错误
     */
    private identifyCriticalErrors(logs: any[]): any[] {
        const errorLogs = logs.filter(log => log.level === 'error');
        if (errorLogs.length === 0) return [];
        
        // 错误消息去重和计数
        const errorMsgCount: { [key: string]: number } = {};
        for (const log of errorLogs) {
            ...
            if (errorMsgCount[msg] >= 3) {
                ...
                if (log) {
                    ...
                    criticalErrors.push({
                        ...
                        suggestion: this.getSuggestionForErrorType(errorType),
    ...
    private findRelatedErrors(fileName: string, fileContent: string): ErrorFileMapping[] {
        ...
        if (analyzedErrors.length === 0) return [];
        ...
        for (const error of analyzedErrors) {
            ...
            // 检查错误消息中是否包含文件名
            let confidence = 0;
            let lineHint: number | undefined = undefined;
     ...
     * 计算错误与文件的关联度
     */
    private calculateErrorFileRelation(errorType: string, errorMsg: string, fileContent: string): number {
        let confidence = 0;
     ...
     * 生成具体的代码修复建议
     */
    private generateCodeFix(error: ErrorFileMapping, fileContent: string, fileExt: string): string | null {
     ...
     * 生成类型错误的修复建议
     */
    private generateTypeErrorFix(error: ErrorFileMapping, fileContent: string): string | null {
        ...
        if (!typeErrorMatch || !typeErrorMatch[1]) return null;
...
if (typeof ${identifier} === 'function') {
     ...
     * 生成引用错误的修复建议
     */
    private generateReferenceErrorFix(error: ErrorFileMapping, fileContent: string): string | null {
        const refErrorMatch = error.errorMessage.match(/(?:is not defined|未定义).*['"]([^'"]+)['"]/i);
        if (!refErrorMatch || !refErrorMatch[1]) return null;
...
if (typeof ${identifier} !== 'undefined') {
     ...
     * 生成语法错误的修复建议
     */
    private generateSyntaxErrorFix(error: ErrorFileMapping, fileContent: string): string | null {
...
// 4. 对象和数组语法是否正确`;
     ...
     * 生成网络错误的修复建议
     */
    private generateNetworkErrorFix(error: ErrorFileMapping, fileContent: string): string | null {
...
function showErrorToUser(message) {
  // 实现错误显示逻辑
     ...
     * 获取错误类型对应的建议
     */
    private getSuggestionForErrorType(errorType: string): string {
        // 定义允许的错误类型
        type ErrorTypeKey = 'type-error' | 'reference-error' | 'syntax-error' | 'network-error' | 
                            'promise-error' | 'dom-error' | 'unknown-error';
        
        // 如果errorType是数字，转换为字符串
        const typeStr = typeof errorType === 'number' ? String(errorType) : errorType;
        
        // 确保错误类型是有效的
        const validatedType = 
            ['type-error', 'reference-error', 'syntax-error', 'network-error', 
            'promise-error', 'dom-error', 'unknown-error'].includes(typeStr) 
            ? typeStr 
            : 'unknown-error';
        
        const suggestions: Record<ErrorTypeKey, string> = {
        ...
        // 使用索引访问，确保类型安全
        return suggestions[validatedType as ErrorTypeKey];
    }
